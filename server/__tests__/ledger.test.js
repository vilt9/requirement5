...
describe('issue and absorb', () => {
  ...
  test('absorb debits, allows overdraft to the floor, and refuses to breach it', async () => {
    ...
    expect(memoryDb.getUserById(user.id).balance).toBe(-80);
    ...
    expect(() => absorb(user.id, 'save', 1000)).toThrow(InsufficientFundsError);
    expect(memoryDb.getCloud().total_absorbed).toBe(380);
    expect(memoryDb.getUserById(user.id).balance).toBeCloseTo(-80, 2);
  });
