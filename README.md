...
- **Video export** — Playwright drives a chrome-free capture route through a
  deterministic flat-to-holo-orbit-to-flat cycle, then ffmpeg stitches the same
  frames into a GIF or MP4. The default outro includes `requirement5.com`;
  render requests can omit the URL while retaining the R5c mark and resistance
  sign-off.
